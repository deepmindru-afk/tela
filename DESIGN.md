...
- **Agent-layer section — faithful app-window skins (2026-06-28, with the user):** the `#agents` chat beat is shown as **two side-by-side, instantly-recognizable app windows — Claude (warm paper, clay sunburst, plain-text replies) and ChatGPT (near-white, gray user bubble, black blossom mark)** — running the *same* wiki exchange, with tela's editor weaving below. The point is recognizability: "Already in Claude and ChatGPT" must read at a glance. These windows are the **one sanctioned exception to "indigo-on-ink, semantic tokens only":** they use dedicated **`--ext-claude-*` / `--ext-gpt-*` / `--ext-ok` skin tokens** (defined once on `:root`, **theme-independent** — a real app window looks the same on either site theme) and a non-tela system-sans typeface, so they read as *other apps*, not tela chrome. Honest, recognizable **approximations** — nominative fair use, **not pixel-cloned logos**; no fabricated dashboards. The token gate still holds (the skins are named tokens, not hardcoded color). Sequential reveal runs in **both** windows in sync; the `commit` turn triggers the weave. Reduced-motion / no-JS → both windows + the finished page render resolved.
