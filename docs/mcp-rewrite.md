...
| Annotation | Tools |
|---|---|
| `ReadOnlyHint` | `list_spaces, list_pages, get_page, search, research, read_chunk, list_backlinks` |
...
- **Resource links in tool results:** `search` / `research` / `get_page`
...
2. **Search results cards** — `search`/`research` returns clickable
...
2. Point everyone at the HTTP transport directly
   (`claude mcp add --transport http tela https://tla.portalos.ru/api/mcp`); local
...
- ✅ Reads (7): list_spaces, list_pages, get_page, list_backlinks, search, research, read_chunk
...
- ✅ Resource links in tool results: get_page, create_page, search, research return `&mcp.ResourceLink` alongside structured output (via `mcpResultWithLinks`, preserving JSON text + structuredContent)
...
- ✅ **Deployed** — `/api/mcp` is LIVE at `https://tla.portalos.ru/api/mcp` (commit cb823b1; verified 401-without-token, route mounted + bearer verifier gating)
...
- ⬜ **WorkOS dashboard (Cagdas):** Connect → Configuration → **Login URI** = `https://tla.portalos.ru/oauth/workos/login`; add **Resource Indicator** `https://tla.portalos.ru/api/mcp`; enable **DCR + CIMD**; register redirect URIs (claude.ai/claude.com/chatgpt).
...
5. Add **Resource Indicator** `https://tla.portalos.ru/api/mcp` (⚠️ without this, `aud` binding silently breaks).
...
- ⬜ Pagination — IN. Cursor + `next_cursor` on `list_pages` / `search` / `research` (tool-result pagination, ours to implement; `list_spaces` stays unpaginated — bounded by membership)
...
- ⬜ **Pagination** — research says implement it (both hosts + token budget reward
  it); cursor on list_pages/search/research. Also truncate
