...
> Status: **Phase 1 + Phase 2 backend done & on `feat/atlas`** — the full generation machinery is ported, wired, tested, and user-reachable: `EngineStore` (Postgres, tested), LLM client (chat→`TELA_LLM_URL`, embed→`rag.Embedder` via the `EmbedFunc` seam), in-process publisher (`atlasPublisher`, tested), run executor + SSE hub + `ResumeDangling`, and the **HTTP API** (source CRUD, trigger, run status, SSE stream) behind `atlasSpaceManageErr`/`requireMembership` (gating tested). `cmd/atlasdiff` is the 1-1 fidelity harness. **Remaining:** (a) the live 1-1 diff run (needs real `TELA_LLM_URL`/`TELA_RAG_EMBED_URL` + a repo + a deployed atlas reference space) and (b) Phase 3 (native operator UI). This is the developer/architecture contract for bringing the standalone `atlas` system natively into tela. It is the source of truth for the build; update it as phases land. End-user docs go to the tela Docs space (space 16) per `CLAUDE.md`.
