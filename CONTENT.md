...
- **Honesty line:** every number (spaces, storage, AI answers/month, Atlas sources, dollar prices) mirrors the backend `plans` table — the source of truth. The AI allowance meters generative LLM calls (ask-your-docs); semantic *search* itself isn't capped. Atlas generation also spends LLM + embedding budget; on the hosted instance it draws on the same managed-AI path, and self-hosting you bring your own LLM + embedder. The Atlas source cap is a published per-tier limit (enforcement at source-create time is a near-term follow-up). Self-serve billing is live (Polar, merchant of record): a paid-plan CTA takes you to in-app checkout; new accounts still start free with the 30-day Plus trial.
