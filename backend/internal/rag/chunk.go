...
type Chunk struct {
	...
	Content     string // raw markdown slice (what we store/return)
	EmbedText   string // contextualised text (what we embed)
}

var headingRE = regexp.MustCompile(`^(#{1,6})\s+(.*\S)\s*$`)

// maxChunkChars caps a section before a forced flush. ~2000 chars ≈ ~500
// tokens — the 2026 recursive-split sweet spot. Heading boundaries flush
// earlier; long sections split into multiple chunks sharing a heading path.
const maxChunkChars = 2000

// ChunkMarkdown splits a page body into heading-aware chunks. Each chunk
// carries the heading breadcrumb it lives under; the page title + breadcrumb is
// folded into EmbedText so every embedded chunk is self-contained (contextual
